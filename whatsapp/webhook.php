...
$originalSidAsistencia = trim((string)($_POST['OriginalRepliedMessageSid'] ?? ''));
$metaAsistencia = wa_obtener_meta_mensaje_respondido($originalSidAsistencia);
$origenAsistencia = (string)($metaAsistencia['origen'] ?? '');
$esRespuestaTemplateAsistencia = ($origenAsistencia === 'template_asistencia_agenda');
$esAsistenciaSi = in_array($buttonPayloadAsistenciaNorm, ['asistencia agenda si', 'asistencia  agenda si'], true)
    || ($esRespuestaTemplateAsistencia && in_array($buttonTextAsistenciaNorm, ['si', 'sí'], true));
$esAsistenciaNo = in_array($buttonPayloadAsistenciaNorm, ['asistencia agenda no', 'asistencia  agenda no'], true)
    || ($esRespuestaTemplateAsistencia && $buttonTextAsistenciaNorm === 'no');
...
if (($userState['step'] ?? '') === 'agenda_confirmar') {
    ...
    if (
        ...
        wa_set_user_state(
            ...
            [
                ...
                'origen_abandono' => 'PRETASACION',
                'motivo_base' => 'NO_AGENDA_REVISION'
            ],
            'ESPERANDO_MOTIVO_CANCELACION_AGENDA',
            'BOT',
            $profileName !== '' ? $profileName : null,
    ...
    if ($fecha === '' || $hora === '') {
        twiml_message_and_save(
            $from,
            "No pude confirmar la agenda porque falta la fecha u hora seleccionada.\n\nVolvé a escribir AGENDAR para intentarlo nuevamente."
        );
