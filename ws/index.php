...
function obtenerContenidoURLConCurl($url) {
	...
	if(validarUsuario($bd, $config->wsInfopelUsuario, $config->wsInfopelContrasena, $config->entorno)){
		...
		switch($_GET['peticion']){
			...
			case 'availability': $resultado = availability($bd); break;
	...
	function availability($bd){	
		...
		for ($i = 0; $i < 15; $i++) {
			...
			if(
				...
				foreach($resp['schedules']['horas_disponibles'] as $h){
					if(isset($h['hora']) && $h['hora'] !== ''){
						$horas[] = $h['hora'];
					}
				}
		...
		if(count($result) <= 0){
			return array("codigo"=>500, "mensaje"=> "No hay disponibilidad.","error"=>500);
		}
