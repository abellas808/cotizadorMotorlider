...
if (!function_exists('curl_init')) {
  jerr("PHP sin cURL (curl_init no existe).");
}
...
function apify_wait_items($datasetId, $token, $limit = 50, $maxSeconds = 90) {
  ...
  while (true) {
    ...
    if ((time() - $start) >= $maxSeconds) {
      return ["ok"=>true, "items"=>[], "count"=>0, "timeout"=>true];
    }
...
list($code, $resp, $curlErr) = apify_post_json($runUrl, $input);
...
$w = apify_wait_items($datasetId, $token, 50, 90);
if (!($w['ok'] ?? false)) {
...
jok(["corrida_id"=>$corridaId, "ml_url"=>$mlUrl, "total_items"=>$total, "items_guardados"=>$guardados]);